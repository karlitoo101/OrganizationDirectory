...
import { Compass, GraduationCap, Sparkles, Bookmark, HelpCircle, Activity, Code, ClipboardList, Menu, X } from 'lucide-react';

interface HeaderProps {
  activeTab: 'explore' | 'quiz' | 'counselor' | 'my-trail' | 'developers' | 'contribute';
  setActiveTab: (tab: 'explore' | 'quiz' | 'counselor' | 'my-trail' | 'developers' | 'contribute') => void;
  bookmarkCount: number;
}

export default function Header({ activeTab, setActiveTab, bookmarkCount }: HeaderProps) {
  const [isOpen, setIsOpen] = useState(false);

  const navItems = [
    { id: 'explore' as const, label: 'Explore', icon: Compass, iconColor: 'text-blue-900' },
    { id: 'quiz' as const, label: 'Matchmaker', icon: Sparkles, iconColor: 'text-amber-500' },
    { id: 'contribute' as const, label: 'Contribute', icon: ClipboardList, iconColor: 'text-teal-500' },
    { id: 'developers' as const, label: 'Creators', icon: Code, iconColor: 'text-rose-500' },
    { id: 'my-trail' as const, label: 'Saved Trail', icon: Bookmark, iconColor: 'text-pink-500', isSaved: true },
  ];
